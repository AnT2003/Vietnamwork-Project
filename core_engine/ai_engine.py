...
def get_embed_model():
    ...
    if embed_model is None:
        print("⏳ Đang nạp Mô hình AI vào RAM... Vui lòng đợi vài giây...")
        from sentence_transformers import SentenceTransformer
        embed_model = SentenceTransformer(EMBEDDING_MODEL_NAME)
        print("✅ Đã sẵn sàng phục vụ AI!")
    return embed_model
...
1. "job_title": Trích xuất chức danh công việc. Nếu không có, để rỗng "".
2. "locations": Mảng các địa phương tìm việc kèm từ đồng nghĩa. Nếu không đề cập, để mảng rỗng [].
...
def fetch_and_rank_jobs(user_query, cv_text, top_k=10):
    ...
    job_title_raw = intent.get('job_title', '')
    job_kw = str(job_title_raw).strip().lower() if job_title_raw else ""
    loc_list = intent.get('locations', [])
    target_synonyms = [str(l).strip().lower() for l in loc_list if l] if loc_list else []
            
    search_query = f"{job_kw} {user_query}"
    if cv_text: search_query += f" {' '.join(cv_text[:500].split())}"
    
    model = get_embed_model()
    query_vector = model.encode(nlp_transform_text(search_query)).tolist()
        ...
        if job_kw:
            ...
            , KeywordSearch AS (
                SELECT CAST(d.job_id AS VARCHAR) AS job_id, d.job_title, c.company_name, d.salary_text, d.job_url, v.chunk_text,
                       CAST(0.0 AS FLOAT) AS vector_distance
    ...
    stopwords = ['tìm', 'việc', 'làm', 'ở', 'tại', 'cho', 'có', 'cần', 'những', 'một', 'vị trí']
...
def generate_llm_response(user_query, jobs_df, cv_text, chat_history, fallback_triggered, requested_loc):
    ...
    system_prompt = f"Bạn là AI Headhunter chuyên nghiệp của VietnamWorks.\n[DỮ LIỆU TỪ HỆ THỐNG]:\n{context_str if context_str else 'Trống.'}\n"
    if fallback_triggered:
        system_prompt += f"\n⚠️ LỖI: HIỆN KHÔNG CÓ CÔNG VIỆC TẠI ({requested_loc.upper()}). Các công việc ở trên nằm ở khu vực KHÁC.\n"

    system_prompt += """
[LUẬT VẼ BẢNG (TABLE) - BẮT BUỘC TUÂN THỦ 100%]:
1. TRUNG THỰC: CHỈ dùng các công việc trong [DỮ LIỆU]. KHÔNG tự bịa data.
2. BẮT BUỘC CHÈN LINK: Cú pháp: **[Tên Vị Trí](Đường_Link_Của_Job)**.
3. TUYỆT ĐỐI KHÔNG BẤM ENTER (XUỐNG DÒNG) BÊN TRONG Ô CỦA BẢNG. Mọi nội dung của 1 hàng (row) PHẢI nằm trên cùng 1 dòng.
4. NẾU MUỐN XUỐNG DÒNG TRONG Ô: Bắt buộc dùng thẻ `<br>`. Ví dụ chuẩn: `| Cột 1 | Ý 1 <br> Ý 2 |`
"""
    ...
    except Exception as e: return f"⚠️ Lỗi kết nối tới AI Model: {e}"